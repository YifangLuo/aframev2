...
class SaveAugmentedBatch(Callback):
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        print(f"saving for batch.")
        print(f"saving for batch:{trainer.global_step}")
        device = pl_module.device
        save_dir = trainer.logger.save_dir
        ...
        [background, _, _], [signals] = next(iter(trainer.datamodule.val_dataloader()))
        background = background.to(device)
        signals = signals.to(device)
        X_bg, X_inj = trainer.datamodule.build_val_batches(background, signals)

        train_batch_filename = os.path.join(save_dir, f"batch_{batch_idx}.h5")
        with h5py.File(train_batch_filename, "w") as f:
            f["X"] = X.cpu().numpy()
            f["y"] = y.cpu().numpy()

        val_batch_filename = os.path.join(save_dir, f"val_batch_{batch_idx}.h5")
        with h5py.File(val_batch_filename, "w") as f:

...
from typing import Callable, List, NamedTuple, Optional, Sequence, Tuple, Union, Any
import warnings
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # Normal Vision Transformer accepts 3-channel inputs(R,G,B)
        # Convert 2-channels input to 3-channels is viable but meaningless
        # self.conv_layer = nn.Conv1d(in_channels=2, out_channels=3, kernel_size=1, stride=1)
        print("Hello! I am using the Vision Transformer.")
        if conv_stem_configs is not None:
            ...
            self.conv_proj = nn.Conv2d(
                in_channels=2, out_channels=hidden_dim, kernel_size=patch_size, stride=patch_size
            )
    ...
    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        # Vision Transformer accpets standard NCHW inputs
        # Which represents batch_num(n), channel_num(c), height(h) and width(w)
        # Downsampling, using a pooling layer to reduce the dimension
        x = functional.avg_pool1d(
            x, kernel_size=self.downsample_rate, stride=self.downsample_rate, padding=0)
        # Copy another dimension of ipnut data converts 1D data to 2D, to adapt the Vision Transformer
        # But I test it, it is slow and there are no obvious accuracy enhancement
        # So I turned to standard Transformer, which accepts 1D time series data
        x = x.unsqueeze(3)
        x = x.repeat(1, 1, 1, self.image_size)
        # Now this is standard ViT input(NCHW)
        n, c, h, w = x.shape
        p = self.patch_size
